...
class Knowledge {
	...
	public static function index_post( int $post_id, int $collection_id ) {
		...
		$existing = Knowledge_Database::find_source( $collection_id, 'post', $post_id );
		...
		if ( $existing ) {
			...
			Knowledge_Database::delete_chunks_for_source( $source_id );
			Knowledge_Database::update_source( $source_id, [
				'title'        => $post->post_title,
				'content_hash' => $hash,
				'status'       => 'pending',
			] );
		} else {
			$source_id = Knowledge_Database::create_source( [
		...
		// Insert chunks.
		$inserted = Knowledge_Database::insert_chunks( $collection_id, $source_id, $chunks );
		...
		// Update collection.
		Knowledge_Database::recalculate_collection_chunk_count( $collection_id );
		Knowledge_Database::update_collection( $collection_id, [
	...
	public static function index_attachment( int $attachment_id, int $collection_id ) {
		$content = Document_Parser::extract_from_attachment( $attachment_id );

		if ( is_wp_error( $content ) ) {
			// Record the error in the source.
			$existing = Knowledge_Database::find_source( $collection_id, 'attachment', $attachment_id );
			if ( $existing ) {
				Knowledge_Database::update_source( (int) $existing->id, [
					'status'        => 'error',
	...
	public static function search( string $query, array $options = [] ): array {
		...
		if ( ! $collection_id && ! empty( $options['collection'] ) ) {
			$col = Knowledge_Database::get_collection_by_slug( $options['collection'] );
			if ( $col ) {
				$collection_id = (int) $col->id;
			}
		}

		$raw_results = Knowledge_Database::search_chunks( $query, $collection_id, $limit );
	...
	public static function delete_source( int $source_id ): bool {
		return Knowledge_Database::delete_source( $source_id );
	}
}
