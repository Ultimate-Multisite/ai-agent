...
class Event_Trigger_Handler {
	...
	public static function attach_hooks(): void {
		$events = Event_Automations::list( true );
		...
		foreach ( $events as $event ) {
			...
			$trigger_def = Event_Trigger_Registry::get( $hook_name );
	...
	private static function dispatch( string $hook_name, array $hook_args ): void {
		...
		foreach ( $events as $event ) {
			...
			$prompt = Placeholder_Resolver::resolve(
	...
	public static function execute_event_run( string $run_key ): void {
		...
		$event = Event_Automations::get( $event_id );
		if ( ! $event ) {
		...
		$loop   = new Agent_Loop( $prompt, [], [], $options );
		$result = $loop->run();
		...
		Event_Automations::record_run( $event_id );
