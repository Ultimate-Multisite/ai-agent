...
export default function AutomationsManager() {
	...
	const fetchAll = useCallback( async () => {
		try {
			const [ result, tpl, prof ] = await Promise.all( [
				...
				apiFetch( { path: '/gratis-ai-agent/v1/automation-templates' } ),
	...
	const handleSubmit = useCallback( async () => {
		...
		try {
			...
			resetForm();
			fetchAll();
			setNotice( { status: 'success', message: __( 'Automation saved.', 'gratis-ai-agent' ) } );
		} catch ( err ) {
			setNotice( { status: 'error', message: err.message || __( 'Failed to save.', 'gratis-ai-agent' ) } );
		}
	...
	const handleDelete = useCallback( async ( id ) => {
		...
		if ( window.confirm( __( 'Delete this automation?', 'gratis-ai-agent' ) ) ) {
			await apiFetch( {
		...
		try {
			...
			setNotice( {
				...
				message: result.success
					? __( 'Automation ran successfully.', 'gratis-ai-agent' )
					: ( result.error || __( 'Automation completed with errors.', 'gratis-ai-agent' ) ),
			} );
			fetchAll();
		} catch ( err ) {
	...
	return (
		<div className="gratis-ai-agent-automations-manager">
			<div className="gratis-ai-agent-skill-header">
				<div>
					<h3>{ __( 'Scheduled Automations', 'gratis-ai-agent' ) }</h3>
					<p className="description">
						{ __( 'Cron-based AI tasks that run on a schedule.', 'gratis-ai-agent' ) }
					</p>
				</div>
				{ ! showForm && (
					<Button
			...
			{ ! showForm && templates.length > 0 && automations.length === 0 && (
				<div style={ { marginBottom: '16px' } }>
					<h4>{ __( 'Quick Start Templates', 'gratis-ai-agent' ) }</h4>
					<div className="gratis-ai-agent-skill-cards">
						{ templates.map( ( tpl, idx ) => (
							<div key={ idx } className="gratis-ai-agent-skill-card">
								...
								<div className="gratis-ai-agent-skill-card-footer">
									...
									>
										{ __( 'Use Template', 'gratis-ai-agent' ) }
									</Button>
								</div>
							</div>
						) ) }
			...
			{ showForm && (
				<div className="gratis-ai-agent-skill-form">
					...
					<TextareaControl
						...
						rows={ 6 }
						help={ __( 'The instruction sent to the AI when this automation runs.', 'gratis-ai-agent' ) }
					/>
					<SelectControl
					...
					<SelectControl
						...
						help={ __( 'Restrict which tools this automation can use.', 'gratis-ai-agent' ) }
						__nextHasNoMarginBottom
					/>
					<TextControl
					...
					<div className="gratis-ai-agent-skill-form-actions">
						<Button
							...
							size="compact"
						>
							{ editId ? __( 'Update', 'gratis-ai-agent' ) : __( 'Create', 'gratis-ai-agent' ) }
						</Button>
			...
			{ ! loaded && (
				<p className="description">{ __( 'Loading...', 'gratis-ai-agent' ) }</p>
			) }

			{ loaded && automations.length > 0 && (
				<div className="gratis-ai-agent-skill-cards" style={ { marginTop: '16px' } }>
					{ automations.map( ( auto ) => (
						...
						>
							...
							<div className="gratis-ai-agent-skill-card-footer">
								<span className="gratis-ai-agent-skill-word-count">
									{ auto.run_count }{ ' ' }{ __( 'runs', 'gratis-ai-agent' ) }
									{ auto.last_run_at && (
										<> &middot; { __( 'Last:', 'gratis-ai-agent' ) } { auto.last_run_at }</>
									) }
								</span>
								<div className="gratis-ai-agent-skill-card-actions">
									...
									>
										{ running === auto.id ? <Spinner /> : __( 'Run Now', 'gratis-ai-agent' ) }
									</Button>
									<Button
										variant="tertiary"
										size="small"
									...
									>
										{ viewLogsId === auto.id ? __( 'Hide Logs', 'gratis-ai-agent' ) : __( 'Logs', 'gratis-ai-agent' ) }
									</Button>
									<Button
										icon={ pencil }
										size="small"
										label={ __( 'Edit', 'gratis-ai-agent' ) }
										onClick={ () => handleEdit( auto ) }
									/>
									<Button
										icon={ trash }
										size="small"
										label={ __( 'Delete', 'gratis-ai-agent' ) }
										isDestructive
							...
							{ viewLogsId === auto.id && (
								<div className="gratis-ai-agent-automation-logs">
									{ logs.length === 0 && (
										<p className="description">{ __( 'No logs yet.', 'gratis-ai-agent' ) }</p>
									) }
									{ logs.map( ( log ) => (
										<div key={ log.id } className={ `gratis-ai-agent-log-entry gratis-ai-agent-log--${ log.status }` }>
											<div className="gratis-ai-agent-log-meta">
												<span className={ `gratis-ai-agent-log-status gratis-ai-agent-log-status--${ log.status }` }>
													{ log.status }
												</span>
											...
											{ log.error_message && (
												<p className="gratis-ai-agent-log-error">{ log.error_message }</p>
											) }
											{ log.reply && (
												<details>
													<summary>{ __( 'Response', 'gratis-ai-agent' ) }</summary>
													<pre className="gratis-ai-agent-log-reply">{ log.reply }</pre>
												</details>
											) }
										</div>
									) ) }
